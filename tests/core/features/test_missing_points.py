...
import pandas as pd
import numpy as np
from interpreTS.core.features.feature_missing_points import missing_points  # Zamień "your_module" na nazwę pliku z funkcją
...
def test_missing_points_mixed_types():
    ...
    percentage_result = missing_points(data)
    count_result = missing_points(data, percentage=False)
    assert percentage_result == 0.2, "Percentage of missing points should be ~33.33%"
    assert count_result == 1, "Missing points count should be 2"
